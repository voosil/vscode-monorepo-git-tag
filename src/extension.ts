...
export function activate(context: vscode.ExtensionContext): void {
    console.log('恭喜，您的插件 "monorepo-git-tag" 已被激活！');

    // 注册命令
    const disposable = vscode.commands.registerCommand('monorepo-git-tag.createTag', async () => {
        try {
            // 获取工作区根目录
            ...
            // 创建并推送tag
            const success = await createAndPushTag(selectedApp.name, newVersion, commitId, commitMessage, workspaceRoot);

            if (success) {
                vscode.window.showInformationMessage(`成功创建并推送tag: @${selectedApp.name}/${newVersion}`);
            } else {
