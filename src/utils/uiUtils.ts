 ...
 */
export async function selectVersionIncrementType(): Promise<'major' | 'minor' | 'patch' | undefined> {
    const options = [
 ...
 */
export async function confirmVersion(currentVersion: string, incrementType: 'major' | 'minor' | 'patch'): Promise<string | undefined> {
 ...
 */
export async function selectCommitId(_defaultCommitId: string, workspaceRoot: string): Promise<string | undefined> {
    // 导入 gitUtils 模块
    const gitUtils = await import('./gitUtils.js');

    // 获取最近的commit列表
    const recentCommits = await gitUtils.getRecentCommits(workspaceRoot, RECENT_COMMITS_COUNT);
    ...
    // 添加自定义commit ID选项
    options.push({ label: '自定义commit ID', description: '手动输入commit ID', detail: '' });
